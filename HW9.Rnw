...
gamma = function(par, data, neg = FALSE){
  alpha = exp(par[1])
  beta = exp(par[2])
  loglik = sum(dgamma(data, shape = alpha, scale = beta, log = TRUE))
  return(ifelse(neg, -loglik, loglik))
}
gamma.mle = optim(par = c(0,0), fn = gamma, data = x, neg = TRUE)
alpha.hat = exp(gamma.mle$par[1])
beta.hat = exp(gamme.mle$par[2])
...
lognormal = function(par, data, neg = FALSE){
  mu = par[1]
  sigma = exp(par[2])
  loglik = sum(dlnorm(data, meanlog = mu, sdlog = sigma, log = TRUE))
  return(ifelse(neg, -loglik, loglik))
}
lognorm.mle - optim(par = c(0,0), fn = lognormal, data = x, neg = TRUE)
